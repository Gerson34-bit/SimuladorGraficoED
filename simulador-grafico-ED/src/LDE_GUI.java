...
public class LDE_GUI extends JDialog {
	...
	private JLabel[] setas2 = new JLabel[9];


	public static void main(String[] args){
        try{
        	LDE_GUI dialog = new LDE_GUI();
...
public LDE_GUI(){
        ...
        this.setTitle("Lista Duplamente Encadeada");
    ...
    private void configurarSetas() {
        for (int i = 0; i < 9; i++) {
            ...
            setas[i].setBounds(220 + i * 153, 280, 73, 72);
    ...
    private void configurarSetas2() {
        for (int i = 0; i < 9; i++) {
            setas[i] = new JLabel("←");  // Usamos "→" como a representação da seta
            setas[i].setBounds(220 + i * 153, 320, 73, 72);
            setas[i].setFont(new Font("Courier", Font.BOLD, 40));
            setas[i].setForeground(Color.RED);
            contentPane.add(setas[i]);
        }
        contentPane.repaint();
    }
